...
class HostedLozzaxAPIClient {
  ...
  SubmitRawTx (req_params, fn) {
    ...
    if (self.context.HostedLozzaxAPIClient_DEBUGONLY_mockSendTransactionSuccess === true) {
      if (self.context.isDebug === true) {
        ...
        throw Error(`[${self.constructor.name}/SubmitSerializedSignedTransaction]: context.HostedLozzaxAPIClient_DEBUGONLY_mockSendTransactionSuccess was true despite isDebug not being true. Set back to false for production build.`)
      }
    }
    net_service_utils.AddUserAgentParamters(
      req_params,
...
module.exports = HostedLozzaxAPIClient

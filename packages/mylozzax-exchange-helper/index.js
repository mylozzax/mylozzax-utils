...
class ExchangeHelper {
    ...
    setSendingFee(feeString, elemToSet) {
        ...
        elemToSet.innerHTML = `<span class="field_title form-field-title" style="margin-top: 8px; color: rgb(158, 156, 158); display: inline-block;">+ <span id='feeString'>${feeString}</span> LOZZ EST. FEE</span>`
    }
    ...
    UnlockedBalance_FormattedString(wallet) { 
        ...
        return lozzax_amount_format_utils.formatMoney(balance_JSBigInt)
    }
    ...
    walletSelectorTemplate(walletList) {
        ...
        if (walletDiv !== null && walletDiv.dataset.walletchosen == "true") {
            ...
            walletBalance.innerText = `${self.UnlockedBalance_FormattedString(walletList[selectorOffset])} LOZZ`;
        } else {
            let walletOptions = ``;
            ...
            <div data-walletoffset="0" data-walletpublicaddress="${defaultWallet.public_address}" data-walletLabel="${defaultWallet.walletLabel}" data-swatch="${defaultWallet.swatch.substr(1)}" data-walletbalance="${self.UnlockedBalance_FormattedString(defaultWallet)}" data-walletid="${defaultWallet._id}" id="selected-wallet" class="hoverable-cell utility selectionDisplayCellView" style="">
                    ...
                    <div id="selected-wallet-balance" class="description-label">${self.UnlockedBalance_FormattedString(defaultWallet)} LOZZ   </div>
                </div>
    ...
    sendFundsValidationStatusCallback (str) {
        const lozzaxdUpdates = document.getElementById('lozzaxd-updates')
        lozzaxdUpdates.innerText = str
    }

function sendFunds (wallet, lozz_amount, lozz_send_address, sweep_wallet, validation_status_fn, handle_response_fn) {
  return new Promise((resolve, reject) => {
    // for debug, we use our own lozz_wallet and we send a tiny amount of LOZZ. Change this once we can send funds

    const enteredAddressValue = lozz_send_address // ;
    ...
    let contact_address
    const raw_amount_string = lozz_amount // LOZZ amount in double
...
export default { getTimeRemaining, isValidBase10Decimal, checkDecimals, renderOrderStatus, sendFunds }

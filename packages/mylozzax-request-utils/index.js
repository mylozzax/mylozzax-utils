...
function New_ParsedPayload_FromPossibleRequestURIString (string, nettype, lozzax_utils/* pass this so this fn remains sync */) {
  // throws; -> {}
  //
  // detect no-scheme lozzaxAddr and possible OA addr - if has no lozzax: prefix
  if (string.indexOf(coinUriPrefix) !== 0) {
    ...
    const couldBeOAAddress = string.indexOf('.') != -1 // contains period - would be nice to get this from DoesStringContainPeriodChar_excludingAsLOZZAddress_qualifyingAsPossibleOAAddress so maybe mylozzax_core_js should gain local_modules/OpenAlias
    if (couldBeOAAddress) {
      return {
        address: string
      }
    ...
    try {
      address__decode_result = lozzax_utils.decode_address(string, nettype)
    } catch (e) {
      throw Error('No Lozzax request info')
    }
    // then it looks like a lozzax address
    return {
      address: string
    }
  }
  ...
  if (protocol !== coinUriPrefix) {
    throw Error('Request URI has non-Lozzax protocol')
  }

...
const MyLozzaxBridge_utils = require('@mylozzax/mylozzax-bridge-utils')
	...
	function locateFile(filename, scriptDirectory)
	{
		...
		if (ENVIRONMENT_IS_NODE) {
			...
			if (typeof __dirname !== undefined && __dirname !== "/") { // looks like node running in browser.. (but not going to assume it's electron-renderer since that should be taken care of by lozzax_utils.js itself)
				...
				pathTo_cryptonoteUtilsDir = `/assets/` // this works for the MyLozzax browser build, and is quite general, at least
			}

...
const ExchangeLibrary = require('@mylozzax/mylozzax-exchange')
...
let exchangeXmrDiv = document.getElementById('exchange-lozz');
...
walletSelectorClickListener = function(event, exchangeElements) {
    ...
    if (event.srcElement.parentElement.className.includes("optionCell")) {
        ...
        walletBalance.innerText = dataAttributes.walletbalance + " LOZZ";
        ...
        try {
            ...
            exchangeElements.currencyInputTimer = setTimeout(() => {
                ...
                inCurrencyGetOffer(inCurrencyDiv, outCurrencyDiv, inCurrencyValue, exchangeElements).then((response) => {
                    ...
                    exchangeElements.minimumFeeText.innerText = `~ ${estimatedTotal} LOZZ EST. TOTAL`;
                    resolve(response);
                }).catch((error) => {

                    exchangeElements.minimumFeeText.innerText = `${exchangeElements.minimumFeeText.dataset.minimumAmount} LOZZ minimum (excluding tx fee)`
                    reject(error);
                })
            }, 1500)
        } catch (error) {
...
updateMinimumInputValue = function(event, exchangeElements) {
    ...
    Utils.getMinimalExchangeAmount("LOZZ", exchangeElements.outCurrencyTickerCodeDiv.value).then(response => {
        //exchangeElements.minimumFeeText.innerText = response.minAmount + " LOZZ minimum (excluding tx fee)" // use this line when we switch to polling ChangeNow
        exchangeElements.minimumFeeText.innerText = response.data.in_min + " LOZZ minimum (excluding tx fee)"
    }).catch(error => {
...
//                     let lozz_dest_address_elem = document.getElementById('LOZZtoAddress');
...
//             let errorDiv = document.createElement("div");
//             errorDiv.innerText = "LOZZ.to's server is unreachable. Please try again shortly.";
//             validationMessages.appendChild(errorDiv);
//         } else {
//             let errorDiv = document.createElement("div");
